...
#### 1.Seed URLs :- Seed URLs act as the entry point for the crawler.
#### 2.Crawler :- The crawler automatically visits web pages and collects their content and links.
#### 3.Page Storage :- Page storage saves the crawled content for future processing.
#### 4.Indexer :- The indexer converts raw web pages into searchable data.
#### 5.Search Engine :- The search engine helps users find relevant information from the crawled data.
...
# CONCLUSION :
Milestone 1 of the WebScour project successfully focuses on implementing the core functionalities of a web crawler using Python. We designed and developed a controlled crawling system starting from a seed URL, with proper URL management, duplicate handling, link filtering, same-domain restriction, page storage, and retry logic for failed requests.
This milestone helped us understand the fundamental working of web crawling systems and apply best practices such as organized data storage and responsible crawling. The implementation provides a strong foundation for extending WebScour into a distributed crawler and full-fledged search engine in the upcoming milestones.
